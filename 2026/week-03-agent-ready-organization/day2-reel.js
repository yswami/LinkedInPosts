(function () {
  ...
  function renderMotion(time){Array.prototype.forEach.call(document.querySelectorAll(".bob"),function(element,index){var y=Math.sin(time*2.25+index*.75)*.48,tilt=Math.sin(time*1.2+index)*1.25;element.style.transform="translateY("+y.toFixed(3)+"cqw) rotate("+tilt.toFixed(3)+"deg)";});Array.prototype.forEach.call(document.querySelectorAll(".sway-alt"),function(element,index){element.style.transform="rotate("+(Math.sin(time*1.45+index)*1.7).toFixed(3)+"deg)";});var opening=smooth(time/1.5);if(curtainLeft)curtainLeft.style.transform="translateX("+(-3.5*opening).toFixed(3)+"cqw)";if(curtainRight)curtainRight.style.transform="translateX("+(3.5*opening).toFixed(3)+"cqw)";if(spotlight)spotlight.style.opacity=(.30+.56*opening).toFixed(3);var route=clamp((time-15.5)/4.1,0,1);var movingCase=document.querySelector(".moving-case");if(movingCase)movingCase.style.marginLeft=(9*smooth(route)).toFixed(3)+"cqw";var dots=Array.prototype.slice.call(document.querySelectorAll(".route-dots i"));dots.forEach(function(dot,index){dot.style.opacity=String(.24+.76*clamp((time-15.1-index*.18)/.22,0,1));dot.style.transform="scale("+(.75+.4*Math.sin(Math.max(0,time-15.1-index*.18)*7)).toFixed(3)+")";});var alert=document.querySelector(".invoice-alert");if(alert&&time<2.4){var pulse=1+.018*Math.sin(time*10);alert.style.boxShadow="0 2cqw 5cqw rgba(199,93,69,"+(.14+.12*Math.abs(Math.sin(time*7))).toFixed(3)+")";alert.style.transform="scale("+pulse.toFixed(4)+")";}}
