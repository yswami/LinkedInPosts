...
async function main() {
  ...
  const yearRoot = path.resolve(root, "..");
  const sharedTools = path.join(yearRoot, "week-02-project-financial-alerts", "tools");
  const htmlPath = path.join(root, "day1-story.html");
  const outputPath = path.resolve(process.argv[2] || path.join(root, "media", "agent-ready-organization-day1-silent.mp4"));
  const coverPath = path.resolve(process.argv[3] || path.join(root, "media", "agent-ready-organization-day1-cover.jpg"));
  ...
  if (!coverOnly) {
    const encoder = spawnSync("swift", [path.join(sharedTools, "encode-series-frames.swift"), framesDirectory, outputPath, String(frameRate), String(width), String(height)], { stdio: "inherit" });
