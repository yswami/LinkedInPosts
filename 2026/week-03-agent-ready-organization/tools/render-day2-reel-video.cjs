...
async function main() {
  ...
  const htmlPath = path.join(root, "day2-reel.html");
  const outputPath = path.resolve(process.argv[2] || path.join(root, "local-output", "agent-ready-organization-day2-reel-silent.mp4"));
  const coverPath = path.resolve(process.argv[3] || path.join(root, "local-output", "agent-ready-organization-day2-reel-cover.jpg"));
  ...
  const coverTime = positiveNumber(process.env.SERIES_COVER_TIME, 35.4);
  const framesDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "agent-ready-day2-reel-"));
