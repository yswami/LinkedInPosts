...
async function main() {
  ...
  const htmlPath = path.join(root, "day5-story.html");
  const outputPath = path.resolve(process.argv[2] || path.join(root, "media", "project-financial-alert-day5-silent.mp4"));
  const coverPath = path.resolve(process.argv[3] || path.join(root, "media", "project-financial-alert-day5-cover.jpg"));
  ...
  const framesDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "financial-alert-day5-"));
  ...
  try {
    ...
    for (let frame = 0; frame < totalFrames; frame += 1) {
      ...
      await page.screenshot({ path: path.join(framesDirectory, `${String(frame).padStart(5, "0")}.jpg`), type: "jpeg", quality: 92, animations: "disabled" });
      if (frame % Math.round(frameRate * 10) === 0) console.log(`Rendered ${Math.round(time)} seconds`);
    }

    await page.evaluate(() => window.seriesAnimation.renderAt(56));
  ...
  const encoder = spawnSync("swift", [path.join(__dirname, "encode-series-frames.swift"), framesDirectory, outputPath, String(frameRate), String(width), String(height)], { stdio: "inherit" });
