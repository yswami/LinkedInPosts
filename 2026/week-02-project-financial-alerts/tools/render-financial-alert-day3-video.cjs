...
async function main() {
  ...
  const htmlPath = path.join(root, "day3-story.html");
  const outputPath = path.resolve(process.argv[2] || path.join(root, "media", "project-financial-alert-day3-silent.mp4"));
  const coverPath = path.resolve(process.argv[3] || path.join(root, "media", "project-financial-alert-day3-cover.jpg"));
  ...
  const framesDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "financial-alert-day3-"));
  ...
  try {
    ...
    await page.evaluate(() => window.seriesAnimation.renderAt(29));
