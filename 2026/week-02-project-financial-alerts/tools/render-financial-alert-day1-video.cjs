...
async function main() {
  ...
  const outputPath = path.resolve(process.argv[2] || path.join(root, "media", "project-financial-alert-day1-silent.mp4"));
  const coverPath = path.resolve(process.argv[3] || path.join(root, "media", "project-financial-alert-day1-cover.jpg"));
  ...
  const framesDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "financial-alert-day1-"));
  ...
  try {
    ...
    await page.evaluate(() => window.seriesAnimation.renderAt(14.5));
